...
func (m *MatchConn) InsertNewMatch(fromUserId, toUserId string, reqStatus matchEntity.Status) (string, error) {
	query := `
	INSERT INTO match(
		request_from, 
		request_to, 
...
func (m *MatchConn) SelectMatchReqToUserId(userId string) ([]matchEntity.FullUserDTO, error) {
	query := `
	SELECT 
		m.id as match_id,
		u.id as user_id,
	...
	matchs := make([]matchEntity.FullUserDTO, 0)
	for rows.Next() {
...
func (m *MatchConn) GetMatchById(matchId string) (matchEntity.MatchDAO, error) {
	query := `
		SELECT
			id,
			request_from,
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	var matchDAO matchEntity.MatchDAO
...
func (m *MatchConn) UpdateMatchById(matchDAO matchEntity.MatchDAO) error {
	query := `
	UPDATE match SET
		request_status=$1, 
		accepted_at=$2, 
	...
	RETURNING id`
	args := []any{
		matchDAO.RequestStatus,
		matchDAO.AcceptedAt,
		matchDAO.RevealStatus,
		matchDAO.RevealedAt,
		matchDAO.Id,
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	err := m.conn.GetContext(ctx, &matchDAO.Id, query, args...)
	if err != nil {
		var pqErr *pq.Error
		switch {
...
func (*MatchConn) createCandidatematch(row sqlx.ColScanner) (matchEntity.FullUserDTO, error) {
	...
	if err != nil {
		return matchEntity.FullUserDTO{}, err
	}
	...
	for i := range hobbies {
		newMatch.Hobbies = append(newMatch.Hobbies, interestEntity.HobbieDTO{
			Hobbie: hobbies[i],
		})
	}
	for i := range movieSeries {
		newMatch.MovieSeries = append(newMatch.MovieSeries, interestEntity.MovieSerieDTO{
			MovieSerie: movieSeries[i],
		})
	}
	for i := range travels {
		newMatch.Travels = append(newMatch.Travels, interestEntity.TravelDTO{
			Travel: travels[i],
		})
	}
	for i := range sports {
		newMatch.Sports = append(newMatch.Sports, interestEntity.SportDTO{
			Sport: sports[i],
		})
	}
	return newMatch, nil
