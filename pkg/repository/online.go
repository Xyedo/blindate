...
func (o *OnlineCon) InsertNewOnline(on domain.Online) error {
	query := `
	INSERT INTO 
...
func (o *OnlineCon) SelectOnline(userId string) (domain.Online, error) {
	query := `
	SELECT
	...
	if err != nil {
		if errors.Is(err, context.Canceled) {
			return domain.Online{}, common.WrapError(err, common.ErrTooLongAccessingDB)
		}
		if errors.Is(err, sql.ErrNoRows) {
			return domain.Online{}, common.WrapError(err, common.ErrResourceNotFound)
		}
